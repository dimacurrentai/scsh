...
command -v packdiff >/dev/null 2>&1 \
  ...
  || echo "packdiff: not found (optional) — install with \`rustup target add wasm32-unknown-unknown && cargo install packdiff --version 0.9.1 --locked\` to see per-step commit diffs"
