...
pub mod reason {
  ...
  /// The account's usage limit (claude.ai's, or grok's quota dialog) stopped the session and it
  /// will not come back on its own. Not a failure of the task and not an overload: the work was
  /// fine, the quota window ran out, and the ONLY thing that fixes it is the clock. Kept apart from
