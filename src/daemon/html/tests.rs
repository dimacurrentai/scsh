...
fn ui_review_fixes_hold() {
  ...
  // A run parked on a provider usage limit is its own state, its own glyph, and its own
  // colour — never orange (that reads as working) and never purple (that reads as abandoned).
  assert!(js.contains("awaiting_limits:'Awaiting limits'"), "legend label is Awaiting limits");
  assert!(js.contains("awaiting_limits:'\u{29d7}'"), "awaiting limits gets the hourglass glyph");
  assert!(html.contains(".wf-node.wf-awaiting_limits { --accent: var(--magenta); }"), "awaiting limits has its own colour");
  assert!(!html.contains(".wf-node.wf-awaiting_limits { --accent: var(--orange); }"));
